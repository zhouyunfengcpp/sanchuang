...
def get_data(url):
    ...
    try:
        ...
        for li in info:
            ...
            scrappertime = get_time()
            id = str(fltno)+str(scrappertime)
...
#get time
def get_time():
    ScraperTime = time.strftime('%Y-%m-%d %H:%M:%S')
    return ScraperTime
    ...
    for depart_city_num in range(0,4):
        for arrive_city_num in range(depart_city_num+1,4):
            ...
            while count<=40:
                ...
                data = get_data(url)
                count = count +1
                day = datetime.date.today() + datetime.timedelta(days=count)
                month_num = day.month-1
                day_num = day.day-1
                print('success get data')
                if data!=False and data!=True: 
                    for each in data:
                        sql.insert_data(each[0], each[1], each[2], each[3], each[4], each[5], each[6], each[7], batch,each[8])
                elif data==True:
                    data=get_data(url)
                    if data!=False and data!=True: 
                        for each in data:
